...
do_mr_online_one_gene <- function(gene,token,outcome,pval){
  ...
  print("Set up proxy settings")
  Sys.setenv(OPENGWAS_JWT = token)
  ieugwasr::get_opengwas_jwt()
  ieugwasr::user()
  
  print("Retrieve and filter exposed instrumental variables")
  ...
  if (nrow(exposure_dat) < 1) {
    stop("Less than 1 effective eQTL gene cannot be analyzed by MR")
  }
  
  print("Obtain instrumental variables for the outcome and perform harmonic analysis")
  ...
  print("Perform MR analysis and save data")
  ...
  print("Draw all SNP Gene volcano maps")
  ...
  if (!exists('result')) {
    dir.create('result/')
  } else {print('The result folder already exists')}
  ggsave(filename = 'result/1.volcanal.pdf',width = 6.5,height = 6.5)
  
  print("Sensitivity analysis of target genes")
...
# Define the boundary position of the CIS region
cis_start <- max(1, GENE_START_POS - CIS_WINDOW_BP)
cis_end <- GENE_END_POS + CIS_WINDOW_BP
print(paste("define 'cis' pos: chr", GENE_CHR, ":", cis_start, "-", cis_end))

# Screen all cis SNPs from pQTL data
...
print(paste("find *cis-Lead SNP*: ", lead_snp_rsid, " (P:", lead_snp_row$p_value, ") at chr", lead_chr, ":", lead_pos))


start_pos <- lead_pos - window_bp
end_pos <- lead_pos + window_bp
if(start_pos < 0) { start_pos <- 1 }
print(paste("define: chr", lead_chr, ":", start_pos, "-", end_pos))
...
print(paste("pQTL find", nrow(pqtl_locus_filtered), "SNPs"))
print("fliter Disease...")
disease_locus_filtered <- disease_data_fixed %>%
  dplyr::filter(chromosome == lead_chr & base_pair_location >= start_pos & base_pair_location <= end_pos)
print(paste("Disease find", nrow(disease_locus_filtered), "SNPs"))
print("merge data")
...
if (any(duplicated(merged_data$snp_id))) {
  print("warning: repetitive SNP ID...")
  merged_data <- merged_data[!duplicated(merged_data$snp_id), ]
}
if (nrow(merged_data) < 50) { 
  stop(paste("error：Too few SNPs in total (", nrow(merged_data), ")"))
}
D1 <- list(
